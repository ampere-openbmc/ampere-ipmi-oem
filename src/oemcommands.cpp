 ...
 */
[[maybe_unused]] static bool getBaseBoardFRUAddr(uint16_t &busIdx,
						 uint8_t &addr)
{
	bool retVal = false;
	sd_bus *bus = NULL;
	...
	for (const auto &fruObj : fruObjects) {
		auto fruDeviceInf =
			fruObj.second.find("xyz.openbmc_project.FruDevice");

		if (fruDeviceInf != fruObj.second.end()) {
			auto chassisProperty =
				fruDeviceInf->second.find("CHASSIS_TYPE");

			if (chassisProperty != fruDeviceInf->second.end()) {
				std::string chassisType = std::get<std::string>(
					chassisProperty->second);
				auto busProperty =
					fruDeviceInf->second.find("BUS");
				auto addrProperty =
					fruDeviceInf->second.find("ADDRESS");

				if ((0 == chassisType.compare(
						  chassisTypeRackMount)) &&
				    (busProperty !=
				     fruDeviceInf->second.end()) &&
				    (addrProperty !=
				     fruDeviceInf->second.end())) {
					busIdx = (uint16_t)std::get<uint32_t>(
						busProperty->second);
					addr = (uint8_t)std::get<uint32_t>(
						addrProperty->second);
					retVal = true;
					break;
				}
 ...
 *  @returns - true if successfully, false if fail
 */
static bool getRawFruData(uint16_t busIdx, uint8_t addr,
 ...
 *  @returns - true if successfully, false if fail
 */
static bool updateMACAddInFRU(std::vector<uint8_t> &fruData,
			      std::vector<uint8_t> macAddress)
{
	bool retVal = false;
	bool shiftFlag = false; /*If shifting is needed*/
	uint32_t boardInfoAreaOffset =
		fruData[fru::commonHeader::boardOffset] * 8;
	uint32_t boardInfoAreaLength = fruData[boardInfoAreaOffset + 1] * 8;
	uint8_t checkSumVal = 0;
	uint32_t fieldOffset, shiftOffset = 0;
	int shiftSpace =
		0; /*The byte length (signed) that the remaining fruData will be shifted by*/
	char macAddressStr[fru::board::macAddressLength + 1];
	std::vector<uint8_t>
		shiftBuffer; /*Buffer to temporarily save data to be shifted*/
	...
	if (boardInfoAreaOffset != 0) {
		...
		for (uint32_t i = 0; i < fru::board::predefinedFieldNum; i++) {
			fieldOffset += (fruData[fieldOffset] &
					fru::fieldLengthByteMask) +
				       1;
		}

		shiftOffset = fieldOffset;

		if (fruData[fieldOffset] !=
		    fru::endOfFieldByte) // not C1 - not EndOfField
		{
			uint8_t boardExtraOneFieldLength =
				fruData[fieldOffset] & fru::fieldLengthByteMask;
			if (!boardExtraOneFieldLength) {
				...
				shiftOffset++;
				shiftFlag = true;
			} else if (boardExtraOneFieldLength !=
				   fru::board::macAddressLength) {
				/*
	...
	if (shiftFlag) {
		shiftSpace = fru::board::macAddressLength + 1 -
			     (shiftOffset - fieldOffset);
		shiftBuffer.assign(fruData.begin() + shiftOffset,
				   fruData.end());
	...
	fruData[fieldOffset] =
		fru::board::macAddressLength | fru::fieldTypeMask;
	sprintf(macAddressStr, "%02X:%02X:%02X:%02X:%02X:%02X", macAddress[0],
    ...
    */
	...
	if (shiftFlag) {
		...
		if (shiftSpace % 8) {
			...
			uint32_t shiftRedundancy =
				shiftSpace >= 0 ? shiftSpace % 8 :
						  8 - std::abs(shiftSpace % 8);
			uint32_t zeroCount = 0,
				 boardInfoAreaRemainingLength =
					 boardInfoAreaLength -
					 (shiftOffset - boardInfoAreaOffset);
			...
			zeroCount = boardInfoAreaRemainingLength -
				    paddingOffset - 1;
			if (zeroCount >= shiftRedundancy) {
				shiftBuffer.erase(
					shiftBuffer.begin() + paddingOffset,
					shiftBuffer.begin() + paddingOffset +
						shiftRedundancy);
			...
			/* Use zero padding if shift redundancy can not be compensated */
				...
				uint32_t shiftPadding =
					shiftSpace >= 0 ? 8 - (shiftSpace % 8) :
							  std::abs(shiftSpace);
		...
		fruData[boardInfoAreaOffset + 1] =
			(uint8_t)boardInfoAreaLength / 8;

		uint32_t productInfoAreaOffset =
			fruData[fru::commonHeader::productOffset] * 8;
		if (productInfoAreaOffset) {
			/* Re-calculate Product Info Area Starting Offset */
			fruData[fru::commonHeader::productOffset] +=
				shiftSpace / 8;
		}

		uint32_t multiRecAreaOffset =
			fruData[fru::commonHeader::multiRecOffset] * 8;
		if (multiRecAreaOffset) {
			/* Re-calculate Multi-Record Area Starting Offset */
			fruData[fru::commonHeader::multiRecOffset] +=
				shiftSpace / 8;
		}
		/*
        ...
        */
		if (productInfoAreaOffset || multiRecAreaOffset) {
			for (uint32_t i = 0; i < fru::commonHeader::length - 1;
			     i++) {
		...
		fruData.insert(fruData.end(), shiftBuffer.begin(),
			       shiftBuffer.end());
	}
	/*
 ...
 *  @returns - true if successfully, false if fail
 */
static bool writeFruData(uint16_t busIdx, uint8_t addr,
			 std::vector<uint8_t> &fruData)
{
	bool retVal = false;
	sd_bus *bus = NULL;
...
ipmi::RspType<> ipmiDocmdConfigureUartSwitch(uint8_t consPort, uint8_t dirSw)
{
	...
	try {
        ...
        */
		cmd = "/usr/sbin/ampere_uartmux_ctrl.sh " + nConsPort + " " +
		      nDirSw;
 ...
 */
std::optional<std::uint16_t> scpReadRegisterMap(std::string path,
						uint8_t offsetR)
{
	uint16_t addrInt;
	std::fstream file;
 ...
 *  @returns - None.
 */
static bool scpWriteRegisterMap(std::string path, uint8_t offsetW,
				uint16_t dataW)
{
	std::fstream file;
...
ipmi::RspType<uint8_t> ipmiSetFanSpeed(uint8_t fanNumber, uint8_t speed)
{
	...
	try {
		...
		cmd = "ampere_fanctrl.sh setspeed " + fanNumberStr + " " +
		      speedStr;
 ...
 */
ipmi::RspType<> ipmiSetFWInbandUpdateStatus(ipmi::Context::ptr ctx,
					     ...
					     uint8_t fwAux3rd, uint8_t fwAux4th)
{
	...
	try {
		...
		try {
			...
			dataIn = { fwMajor,  fwMinor,  fwAux4th,
				   fwAux3rd, fwAux2nd, fwAux1st };
 ...
 */
static ipmi::RspType<> setSoCPowerLimit([[maybe_unused]] ipmi::Context::ptr ctx,
					uint8_t upper, uint8_t lower)
{
	...
	if (!powerLimitJsonData.is_discarded()) {
		...
		if ((powerLimitJsonData.contains("SoC")) &&
			...
			supportFlg = true;
			auto pldmSensors =
				powerLimitJsonData.at("SoC").at("pldm");
			auto sdBus = getSdBus();
			...
			if (pldmSensors.is_array()) {
				...
				for (const auto &entity : pldmSensors) {
					...
					try {
                         ...
                         */
						double maxVal = std::get<
							double>(ipmi::getDbusProperty(
							*sdBus, pldmService,
							ojctPath.c_str(),
							pldmSensorValInterface,
							pldmSensorMaxValPro));

						double minVal = std::get<
							double>(ipmi::getDbusProperty(
							*sdBus, pldmService,
							ojctPath.c_str(),
							pldmSensorValInterface,
 ...
 *          0xFF: An error occurs.
 */
static ipmi::RspType<> setDRAMMaxThrottleEnable(ipmi::Context::ptr ctx,
						uint8_t status)
{
	ipmi::Cc completeCode = ipmi::ccSuccess;
	bool supportFlg = false;
	...
	if (!powerLimitJsonData.is_discarded()) {
		...
		if ((powerLimitJsonData.contains("DRAM")) &&
			...
			supportFlg = true;
			auto pldmSensors =
				powerLimitJsonData.at("DRAM").at("pldm");
...
void registerOEMFunctions()
{
	...
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
			      ipmi::general::cmdScpWrite,
			      ipmi::Privilege::Admin,
			      ipmiDocmdScpWriteRegisterMap);
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
			      ipmi::general::cmdEditBmcMacAdr,
			      ipmi::Privilege::Admin, ipmiDocmdSetMacAddress);
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
	...
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
			      ipmi::general::cmdSetFanSpeed,
			      ipmi::Privilege::Admin, ipmiSetFanSpeed);
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
			      ipmi::general::cmdSetFWInbandUpdateStatus,
			      ipmi::Privilege::Admin,
			      ipmiSetFWInbandUpdateStatus);
	ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::ampere::netFnAmpere,
