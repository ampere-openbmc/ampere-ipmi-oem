...
{
	static constexpr const char *selEraseTimestamp =
		"/var/lib/ipmi/sel_erase_time";

	void save()
	{
		// open the file, creating it if necessary
		int fd = open(selEraseTimestamp, O_WRONLY | O_CREAT | O_CLOEXEC,
			      0644);
		if (fd < 0) {
...
namespace storage
{
	...
	boost::container::flat_map<uint8_t, std::pair<uint8_t, uint8_t> >
		deviceHashes;

	void registerStorageFunctions() __attribute__((constructor));

	static bool
	getSELLogFiles(std::vector<std::filesystem::path> &selLogFiles)
	{
		// Loop through the directory looking for ipmi_sel log files
		for (const std::filesystem::directory_entry &dirEnt :
		     std::filesystem::directory_iterator(
			     ampere_oem::ipmi::sel::selLogDir)) {
			std::string filename = dirEnt.path().filename();
			if (boost::starts_with(
				    filename,
				    ampere_oem::ipmi::sel::selLogFilename)) {
	...
	static bool
	findSELEntry(const int recordID,
	...
	static int fromHexStr(const std::string &hexStr,
			      std::vector<uint8_t> &data)
	{
		for (unsigned int i = 0; i < hexStr.size(); i += 2) {
			try {
				data.push_back(static_cast<uint8_t>(std::stoul(
					hexStr.substr(i, 2), nullptr, 16)));
	...
	ipmiStorageGetSELInfo()
	{
		...
		uint32_t eraseTimeStamp =
			ampere_oem::ipmi::sel::erase_time::get();
		...
		return ipmi::responseSuccess(selVersion, entries, freeSpace,
					     addTimeStamp, eraseTimeStamp,
					     operationSupport);
	}
	...
	using systemEventType = std::tuple<
		...
		std::array<uint8_t,
			   ampere_oem::ipmi::sel::systemEventSize> >; // Event Data
	using oemTsEventType = std::tuple<
		uint32_t, // Timestamp
		std::array<uint8_t,
			   ampere_oem::ipmi::sel::oemTsEventSize> >; // Event Data
	using oemEventType =
		std::array<uint8_t,
			   ampere_oem::ipmi::sel::oemEventSize>; // Event Data
			       ...
			       uint8_t offset, uint8_t size)
	{
		...
		if (recordType == ampere_oem::ipmi::sel::systemEvent) {
			...
			if (targetEntryFields.size() >= 6) {
				...
				try {
					generatorID = std::stoul(generatorIDStr,
								 nullptr, 16);
				} catch (const std::invalid_argument &) {
			...
			std::array<uint8_t,
				   ampere_oem::ipmi::sel::systemEventSize>
				eventData{};
			...
			return ipmi::responseSuccess(
				...
				systemEventType{ timestamp, generatorID, evmRev,
						 sensorType, sensorNum,
						 eventType, eventDir,
						 eventData });
		} else if (recordType >=
				   ampere_oem::ipmi::sel::oemTsEventFirst &&
			   recordType <=
				   ampere_oem::ipmi::sel::oemTsEventLast) {
			// Get the timestamp
			std::tm timeStruct = {};
			std::istringstream entryStream(entryTimestamp);
			...
			std::array<uint8_t,
				   ampere_oem::ipmi::sel::oemTsEventSize>
				eventData{};
			std::copy_n(eventDataBytes.begin(),
				    std::min(eventDataBytes.size(),
					     eventData.size()),
			...
			// Only keep the bytes that fit in the record
			std::array<uint8_t, ampere_oem::ipmi::sel::oemEventSize>
				eventData{};
			std::copy_n(eventDataBytes.begin(),
				    std::min(eventDataBytes.size(),
					     eventData.size()),
		...
		uint8_t eventData2, uint8_t eventData3)
	{
		...
		ampere_oem::ipmi::sel::checkRedfishHooks(
			recordID, recordType, timestamp, generatorID, evmRev,
	...
	ipmiStorageClearSEL(ipmi::Context::ptr, uint16_t reservationID,
			    ...
			    uint8_t eraseOperation)
	{
		...
		static constexpr std::array<uint8_t, 3> clrExpected = { 'C',
									'L',
									'R' };
		if (clr != clrExpected) {
